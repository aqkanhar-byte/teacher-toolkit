...
async function streamFileTo(fileId, res) {
  ...
  return new Promise((resolve, reject) => {
    data.on('end', resolve);
    /* Headers are already sent by the time a mid-stream error can happen, so the caller's own
       error response never reaches the browser — without actively killing the connection here,
       the client's fetch just hangs forever instead of failing (confirmed live: a dropped Drive
       stream left the browser waiting indefinitely with no error, no timeout). Destroying res
       aborts the connection so the browser's fetch/read rejects and the UI can show an error. */
    data.on('error', (err) => { res.destroy(); reject(err); });
    res.on('close', () => { if (!res.writableEnded) data.destroy(); });
    data.pipe(res);
  });
}
